...
const EventList_alumnos_row: React.FC<EventListAlumnosRowProps> = ({data2,navigation}) => {    
  //console.log("datalistalumnos",data)
  const renderRow = ({ item }: { item: alumno[]}) => (
    <View style={styles.row} >
      {item.map((alumno, index) => (   
        <EventItemAlumnosRow key = {alumno.id} alumno_id={alumno.id} firstname={alumno.firstname} lastname={alumno.lastname} navigation={navigation}/>
      ))}
    </View>
  );
