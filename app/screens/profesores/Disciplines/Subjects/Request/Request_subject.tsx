...
const RequestSubjectProfesoresScreen: React.FC<DetailSubjectsProps> = ({ navigation, route }) => {
    ...
    const fetchData2 = async () => {
        try {
            ...
            const response = await axios.get(`https://catolica-backend.vercel.app/apiv1/subjects/${subject_id}/requests/`, {
                headers: {
    ...
    const handleAccept = async (requestId: string) => {
        try {
            const token = await SecureStore.getItemAsync('tikin');
            await axios.patch(`https://catolica-backend.vercel.app/apiv1/subjects/${subject_id}/requests/${requestId}/acceptordeny-subject/`, { accept: true }, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    const handleReject = async (requestId: string) => {
        try {
            const token = await SecureStore.getItemAsync('tikin');
            await axios.patch(`https://catolica-backend.vercel.app/apiv1/subjects/${subject_id}/requests/${requestId}/acceptordeny-subject/`, { accept: false }, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
...
export default RequestSubjectProfesoresScreen
;
